...
const SEARCH_ENGINE_ID = '368951ce04c685cac';
...
function _createQuery(entity, searchFilters,options) {
  ...
  if (entity.value.includes('"')){
    //Assume user is intending to search for something specific with quotes so don't mess with the string
    query = `("${entity.value.replace('so:','')}") AND (`+ options.searchString +')';
  }
  else{
    //Replace spaces with " AND " to drive better logic and avoid unintentional tokenization by google
    query = `("${entity.value.replace('so:','').replace(' ','" AND "')}") AND (`+ options.searchString +')';
  }
    ...
    results.forEach((result) => {
      if (_isMiss(result.body)) {
        ...
        lookupResults.push({
          entity: result.entity,
          data: {
            summary: [`${result.body.searchInformation.totalResults} possible answers`],
...
function validateOptions(userOptions, cb) {
  ...
  if (
    ...
    (typeof userOptions.searchString.value === 'string' && userOptions.searchString.value.length === 0)
  ) {
    errors.push({
      key: 'searchString',
      message: 'Add a search string to narrow search such as "JavaScript"'
    });
  }
  cb(null, errors);
}
